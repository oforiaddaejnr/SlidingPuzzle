...
public interface PuzzleState {
	 ...
	 * If it is, the method creates and returns a new instance of PuzzleState to 
	 * represent the resulting state of the move such that this object still 
	 * represents the current state. In the newly generated and returned state:
	 * the tiles on the board are arranged as they result from the move operation,
	 * the parent state is set to this object,i.e. the current state before the move, 
	 * the operation is set to op, and 
	 * the path length is set to the length of the path from the initial state 
	 * to the newly generated state, 
	 * which is exactly one step more than from the initial state to this state. 
	 * Note that this path length only increases with move operations
	 * regardless of the distance to the initial state.
