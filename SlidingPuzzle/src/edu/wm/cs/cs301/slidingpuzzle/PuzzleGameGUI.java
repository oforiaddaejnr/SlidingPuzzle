...
public class PuzzleGameGUI extends Frame implements ActionListener, MouseListener, MouseMotionListener, ItemListener {
	...
	public void mousePressed(MouseEvent e) {
		...
		Point pixelPosition = e.getPoint();
		if (drag.getState() && isValid(pixelPosition)) {
	...
	public void mouseDragged(MouseEvent e) {
		if (drag.getState()) {
			if(selectedSquare != null) {
				...
				dontChange=true;
				// selects checkSquare if it is valid on the plane, 
				// and not the selected square, 
				// and not an already checked square.
				//This avoids repaint while it is being dragged.
				Point pixelPosition = e.getPoint();
				if (isValid(pixelPosition)) {
					Point gridPosition = getSquareAt(pixelPosition);
	...
	/**
	 * Decides if a given position as pixel coordinates is on the board and not on the frame.
